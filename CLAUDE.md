# Трекер — организация жизни

Личное PWA-приложение Данила «для организации жизни» (себя и жены). Next.js 14 (App Router) + Neon (Postgres) + JWT-авторизация, задеплоено на **Vercel** (тот же проект). Установочное на телефон, тёмная тема, мобайл-фёрст, русский язык интерфейса.
